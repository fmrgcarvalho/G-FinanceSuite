/* ============================================================
   G-FinanceSuite — Op3 UI: render, eventos, tabs e picker.
   Importa a lógica pura de op3Engine.js.
   ============================================================ */

import { AppState } from '../state.js';
import { Logger } from './logger.js';
import { trapFocus, releaseFocus } from './ui.js';
import { listStoredFiles, loadStoredFile, deleteStoredFile, saveFileToStore } from './filestore.js';
import {
  norm, detectCol, FILE_FIELDS, FIELD_PATTERNS,
  parseFile, parseHeaders,
  normalizeSap, normalizeRw, normalizePagosPor, normalizeMapeamento,
  reconcile,
} from './op3Engine.js';
...
const _nav = { list: 'rwNotSap', page: 1 };
...
function renderActiveTable(records, type) {
  ...
  const overrides = AppState.op3.overrides || {};
  const rowsHtml  = type === 'matched'
    ? slice.map(m => {
        const isOverride = !!overrides[m.key];
    ...
    : slice.map(r => {
        const key       = r.atribuicao;
        const isOverride = !!overrides[key];
        const badge = isOverride
          ? `<span style="background:#dbeafe;color:#1d4ed8;padding:2px 8px;border-radius:9999px;font-size:10px;font-weight:600;margin-left:6px">Aceite</span>`
          : `<button data-override-key="${escHtml(key)}" style="margin-left:6px;padding:2px 7px;background:white;border:1px solid #d1d5db;border-radius:5px;font-size:10px;cursor:pointer;color:#374151">Aceitar</button>`;
        return `<tr style="border-bottom:1px solid #f3f4f6">
          ...
          <td style="padding:8px 10px;font-size:12px;font-weight:600">${escHtml(r.atribuicao)}${badge}</td>
  ...
  requestAnimationFrame(() => {
    ...
    // Delegate override button clicks
    table.querySelectorAll('[data-override-key]').forEach(btn => {
      btn.addEventListener('click', () => {
        AppState.op3.overrides[btn.dataset.overrideKey] = { ts: new Date().toISOString() };
        const { records: recs, type: t } = getActiveRecords(AppState.op3.results[AppState.op3.activeTab]);
        renderActiveTable(recs, t);
      });
    });
    renderPag(records, type);
  });
...
function selectList(list) { _nav.list = list; _nav.page = 1; renderActiveTab(); }
...
function updateTabBtns() {
  ['faturacao', 'rmkt', 'pagosPor'].forEach(tab => {
    ...
    const isActive = tab === AppState.op3.activeTab;
    btn.disabled = !hasData;
    btn.classList.toggle('op3-tab-active', isActive);
    btn.setAttribute('aria-selected', isActive ? 'true' : 'false');
    btn.style.opacity = hasData ? '1' : '0.4';
  });
}

function selectTab(tab) { AppState.op3.activeTab = tab; resetNav(); updateTabBtns(); renderActiveTab(); }
...
export function exportOp3Xlsx() {
  const XLSX = window.XLSX;
  if (!XLSX) { alert('⚠️ Biblioteca XLSX ainda a carregar. Tenta novamente.'); return; }
  const results = AppState.op3.results;
  const wb      = XLSX.utils.book_new();
  const overrides = AppState.op3.overrides || {};

  const STD_COLS    = ['ficheiro_origem', 'documento', 'atribuicao', 'montante', 'conta_cliente', 'data'];
  const MATCH_COLS  = ['ficheiro_origem', 'doc_rw', 'doc_sap', 'atribuicao', 'montante'];

  const toStdRows  = arr => arr.map(r => ({ ...r, override: !!overrides[r.atribuicao] }));
  const toMatchRows = arr => arr.map(m => ({ ficheiro_origem: m.rw.ficheiro_origem, doc_rw: m.rw.documento, doc_sap: m.sapDoc, atribuicao: m.rw.atribuicao, montante: m.rw.montante }));

  ['faturacao', 'rmkt', 'pagosPor'].forEach(tab => {
    const r = results[tab];
    if (!r) return;
    const tabLabel = tab === 'pagosPor' ? 'PagosPor' : tab.charAt(0).toUpperCase() + tab.slice(1);
    const wsData = [
      ['== Em RW, não em SAP =='],
      [...STD_COLS, 'override'],
      ...toStdRows(r.somenteRw).map(row => [...STD_COLS, 'override'].map(k => row[k] ?? '')),
      [],
      ['== Em SAP, não em RW =='],
      [...STD_COLS, 'override'],
      ...toStdRows(r.somenteSap).map(row => [...STD_COLS, 'override'].map(k => row[k] ?? '')),
      [],
      ['== Reconciliados =='],
      MATCH_COLS,
      ...toMatchRows(r.matched).map(row => MATCH_COLS.map(k => row[k] ?? '')),
    ];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(wsData), tabLabel);
  });

  if (!wb.SheetNames.length) { alert('Sem dados para exportar.'); return; }
  XLSX.writeFile(wb, `op3_resultados_${new Date().toISOString().substring(0,10)}.xlsx`);
  Logger.info('Op3: XLSX exportado');
}

// ── Executar Op3 — Fase 1: ler cabeçalhos ─────────────────────
...
// ── Executar Op3 — Fase 2: análise completa ────────────────────
  ...
  try {
    ...
    if (f.rwFaturacao || lib.rwFaturacao) {
      ...
      if (!lib.rwFaturacao && f.rwFaturacao) {
        ...
        catch (e) { Logger.warn(`Não foi possível guardar ${f.rwFaturacao.name} — ${e.message}`); }
      }
    ...
    if (f.rwRmkt || lib.rwRmkt) {
      ...
      if (!lib.rwRmkt && f.rwRmkt) {
        ...
        catch (e) { Logger.warn(`Não foi possível guardar ${f.rwRmkt.name} — ${e.message}`); }
      }
    ...
    if (f.rwPagosPor || lib.rwPagosPor) {
      ...
      if (!lib.rwPagosPor && f.rwPagosPor) {
        ...
        catch (e) { Logger.warn(`Não foi possível guardar ${f.rwPagosPor.name} — ${e.message}`); }
      }
...
export async function openOp3LibPicker(slot) {
  ...
  list.onclick = async e => {
    ...
    if (!updated.length) { closeOp3LibPicker(); }
    else { _renderPickerList(updated); }
  };
